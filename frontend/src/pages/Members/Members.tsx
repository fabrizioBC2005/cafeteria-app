...
const PLANS = [
  {
    ...
    badge: null,
    benefits: ["Tarjeta digital de cliente","Codigo QR personalizado","Descuento 10% en consumo","1 cafe gratis cada 10 compras","Acceso a newsletter exclusivo"],
  ...
  },
  {
    ...
    badge: "MAS POPULAR",
    benefits: ["Todo lo de Classic","Reservas prioritarias","Acceso a productos especiales","Descuento 15% en consumo","1 cafe gratis cada 8 compras","Invitaciones a catas y eventos"],
  ...
  },
  {
    ...
    badge: "PREMIUM",
    benefits: ["Todo lo de Gold","Descuento 20% en consumo","1 cafe gratis cada 5 compras","Eventos privados VIP","Envio a domicilio prioritario","Acceso anticipado a lotes nuevos","Atencion personalizada"],
    notIncluded: [],
  },
]

const BENEFITS_GLOBAL = [
  { icon: "💰", title: "Descuento 10%",        desc: "En todos tus consumos dentro del local desde el primer dia." },
  { icon: "☕", title: "Cafe gratis",           desc: "Acumula 1 punto por compra. Al llegar a 10 canjeas un cafe gratis." },
  { icon: "📅", title: "Reservas prioritarias", desc: "Tu mesa siempre disponible. Prioridad sobre clientes sin membresia." },
  { icon: "🌿", title: "Productos especiales",  desc: "Acceso a lotes de edicion limitada antes que el publico general." },
]

function QRDisplay({ qrUrl, codigo }: { qrUrl?: string; codigo: string }) {
  if (qrUrl) {
    return <img src={qrUrl} alt="QR Membresia" style={{ width: 70, height: 70 }} />
  }
  const size = 9
...
function Members() {
  ...
  const handleSuscribir = async (plan?: string) => {
    ...
    try {
      ...
      setSuccess(`Membresia ${planToUse.toUpperCase()} activada!`)
      setTimeout(() => setSuccess(""), 3000)
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { error?: string } } })?.response?.data?.error || "Error al activar membresia"
      setError(msg)
    } finally {
  ...
  const handleCancelar = async () => {
    if (!confirm("Cancelar tu membresia?")) return
    try {
      await membresiaService.cancelar()
      setMembresia(null)
      setSuccess("Membresia cancelada")
      setTimeout(() => setSuccess(""), 3000)
    ...
    <div className="members-page">
      ...
      <div className="members-hero">
        ...
        <div className="members-hero-content">
          <p className="members-eyebrow">LOCAFE / MEMBRESIA</p>
          <h1 className="members-title">COFFEE<br /><span>MEMBERS</span></h1>
          <p className="members-sub">
            Unete al club de los verdaderos amantes del cafe. Beneficios exclusivos,
            tu tarjeta digital personalizada y mucho mas.
          </p>
          ...
          <div className="members-hero-btns">
            {membresia?.activo ? (
              <>
                <button className="members-cta-btn" onClick={() => setFlipped(!flipped)}>Ver mi tarjeta digital</button>
                <button className="members-secondary-btn" onClick={handleCancelar}>Cancelar membresia</button>
              </>
            ) : (
              <>
                ...
                {!isLoggedIn && (
                  <button className="members-secondary-btn" onClick={() => navigate("/register")}>
                    Crear cuenta gratis
                  </button>
                )}
              </>
          ...
          {membresia?.activo && (
            <div style={{ marginTop:24, padding:"16px 20px", background:"rgba(255,255,255,0.08)", borderRadius:8, border:"1px solid rgba(255,255,255,0.15)" }}>
              <p style={{ color:"rgba(255,255,255,0.5)", fontSize:10, letterSpacing:3, marginBottom:8 }}>MI MEMBRESIA</p>
              <div style={{ display:"flex", gap:24, flexWrap:"wrap" }}>
                {[
                  ...
                  { label:"VALIDO",  value: membresia.fecha_fin?.slice(0,10) },
                ].map(item => (
                  <div key={item.label}>
      ...
      <div className="members-benefits-section">
        <div className="members-benefits-inner">
          <p className="section-eyebrow">POR QUE UNIRSE</p>
      ...
      <div className="members-plans-section">
        <div className="members-plans-inner">
          ...
          <p className="section-sub">Todas incluyen tarjeta digital con QR y codigo de cliente unico.</p>
      ...
      <div className="members-digital-section">
        <div className="members-digital-inner">
          <div className="digital-text">
            ...
            <h2 className="digital-title">Tu membresia<br /><span>en tu bolsillo</span></h2>
            <p className="digital-desc">
              Cada miembro recibe una tarjeta digital unica con su codigo QR y numero de cliente.
              Presentala en caja para aplicar tus descuentos y acumular puntos automaticamente.
            </p>
            <ul className="digital-features">
              {["Disponible en tu perfil siempre","Codigo unico e intransferible","Validacion instantanea en caja","Se actualiza con cada compra"].map(f => <li key={f}>{f}</li>)}
            </ul>
          </div>
            ...
            <div className={`card-flip-wrap ${flipped ? "flipped" : ""}`} onClick={() => setFlipped(!flipped)}>
              <div className="card-face card-front" style={{ background: activePlan.gradient }}>
                ...
                <div className="card-face-bottom">
                  ...
                  <div>
                    <p className="card-face-label">VALIDO</p>
              ...
              <div className="card-face card-back" style={{ background: activePlan.gradient }}>
                <p className="card-back-title">CODIGO DE ACCESO</p>
        ...
        <div className="members-cta-inner">
          ...
          <p className="cta-sub">
            {membresia?.activo
              ...
              : "Crea tu cuenta gratis y elige tu membresia en minutos."}
          </p>
          <div className="cta-btns">
            {membresia?.activo ? (
              <button className="members-cta-btn" onClick={() => navigate("/reservas")}>Hacer una reserva</button>
            ) : (
              <>
                <button className="members-cta-btn" onClick={() => handleSuscribir()} disabled={subscribing}>
                  {subscribing ? "Activando..." : "Activar membresia"}
                </button>
                {!isLoggedIn && (
                  <button className="members-secondary-btn white" onClick={() => navigate("/login")}>
                    Ya tengo cuenta
                  </button>
                )}
              </>
